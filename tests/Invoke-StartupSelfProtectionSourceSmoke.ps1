...
Assert-Match $files.SelfProtect 'Startup value is invalid; startup protection remains disabled(?s:.*?)return FALSE' 'invalid startup value fail-open recovery'
Assert-Match $files.SelfProtect 'Startup value is empty; startup protection remains disabled(?s:.*?)return FALSE' 'empty startup value fail-open recovery'
Assert-Match $files.SelfProtect 'Startup command has no executable path; startup protection remains disabled(?s:.*?)return FALSE' 'missing startup path fail-open recovery'
Assert-Match $files.SelfProtect 'Startup path could not be resolved; startup protection remains disabled(?s:.*?)return FALSE' 'unresolvable startup path fail-open recovery'
