...
| R0 behavior | Launch each of the six command-rule fixtures and release one from the full decision window. Confirm the command runs only after release. | Block shadow-copy destruction and process/thread injection fixtures. Confirm process launch fails or dangerous handle rights are stripped. | Stop the bridge, then verify high-confidence command rules fail closed, policy bypass and injection retain their documented infrastructure fallback, and no settings switch can disable the module. | Decision window, event/behavior type, correlation ID, command result, handle mask, driver state module/capability bits. |
